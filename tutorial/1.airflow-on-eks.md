...
좌측 dags 메뉴를 클릭하여 mlops-on-eks 레포지토리의 dags 폴더에 들어있는 DAG 목록이 보이는지 확인한다.
```
kubectl logs airflow-scheduler-594bdd5784-7t6tf -c git-sync -n airflow
```
error: container git-sync is not valid for pod airflow-scheduler-594bdd5784-7t6tf
