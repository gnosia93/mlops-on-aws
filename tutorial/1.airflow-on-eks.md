...
postgresql:
  enabled: true
  image:
    # Docker Hub 대신 AWS Public ECR 이미지 사용 (다운로드 제한 우회)
...
webserver:
  ...
  service:
    type: LoadBalancer
    # 특정 IP에서만 접속을 허용하는 설정 (보안 그룹에 자동 반영)
    loadBalancerSourceRanges:
      - "122.36.213.114/32"
    # AWS 환경에서 권장되는 추가 주석 (선택 사항: NLB 사용 시)
    annotations:
...
```
# Username: admin
kubectl get secret airflow-webserver-password -n airflow -o jsonpath="{.data.password}" | base64 --decode
